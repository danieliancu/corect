...
- Homepage design (`templates/core/home.html`, `static/css/home.css`, `landing.css`):
  ...
  - **Hero (desktop).** The one-line title „Vorbește natural limba engleză” with „limba engleză” in blue, the lead, three value points, a handwritten note in Caveat from Google Fonts (see the Privacy notice) and the Westminster picture fading into the page.
...
- Romanian comes back with the same polite attitude.
...
2. **Shared addresses block `migrate`**: migration `accounts.0003` checks first and stops, changing nothing, if any address is shared. Decide per case which account keeps the address — usually the one that signs in; ask the owners if unsure — and give the others a different address the owner confirms, or delete a clearly abandoned duplicate. Never invent addresses. Then run `migrate` again: it normalises the existing addresses and creates the unique index. On a very large `auth_user` table, create the index beforehand with `CREATE UNIQUE INDEX CONCURRENTLY accounts_user_email_ci_unique ON auth_user (LOWER(email)) WHERE email <> ''` and fake nothing: the migration fails loudly if the index already exists, so drop it again or apply the migration in a maintenance window.
