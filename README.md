...
| `PRO_DISPLAY_PRICE` | Pro price shown on the homepage plans, default `£9.99`. Display only: nothing is charged or enforced |
...
The same command closes live transcription sessions the browser never finished (closed tab, lost network), writing one ledger row each with an unknown cost, and deletes closed session rows after seven days. It never deletes usage ledger rows or visitors.
...
- The privacy page (`/confidentialitate/`) tells visitors about the cookie. Set `ANALYTICS_VISITOR_COOKIE=false` to stop visitor tracking entirely (requests are still counted, without a visitor). Depending on where the app is offered, analytics cookies may require consent; review this for your jurisdiction.
