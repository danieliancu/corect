...
| `LEARNING_AI_DAY_LIMITS` | Learning AI calls per London day per account, `Free,Pro`, default `40,200` |
...
| `EMAIL_BACKEND`, `EMAIL_HOST`, `EMAIL_PORT`, `EMAIL_HOST_USER`, `EMAIL_HOST_PASSWORD`, `EMAIL_USE_TLS`, `EMAIL_USE_SSL`, `EMAIL_TIMEOUT`, `DEFAULT_FROM_EMAIL` | Account emails (verification, password reset, notices). Empty `EMAIL_BACKEND`: console with `DJANGO_DEBUG=true`, SMTP otherwise. See [Accounts, email verification and Google](#accounts-email-verification-and-google) |
| `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET` | "Continuă cu Google"; both empty hides the button |
| `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRO_PRICE_ID`, `STRIPE_TIMEOUT` | Pro subscriptions; all three or none. See [Pro subscriptions (Stripe)](#pro-subscriptions-stripe) |
...
| Pro | Active Stripe subscription, or the manual "Pro" group | 200, the Fair Use ceiling (shown as "Cereri nelimitate (Fair Use)"; the Terms state the ceiling) |

Pro features (`apps/core/entitlements.py`): the learning dashboard, progress, mistakes by category, personalised practice and the full history. A Free account sees a "Disponibil în Pro" screen instead of those pages and its history shows the last 30 days (`FREE_HISTORY_DAYS`); nothing is deleted, and everything appears after an upgrade.
...
- **What people see.** When the allowance is used up the result card shows an alert. Anonymous visitors are invited to create a free account (20 a day); Free users are told that Pro gives unlimited requests under Fair Use („Cu Pro ai cereri nelimitate, în regim Fair Use.”), with a link to the plans; Pro users are told the Fair Use limit resets at midnight UK time. The count of what is left today („19 din 20 de utilizări rămase astăzi”) is shown only on the profile, never under a result; Pro is not counted down. History, progress, mistakes, practice pages and speech already loaded in the page keep working.
...
| Starts checkout / pays | `checkout_started`, `subscription_started` | `checkout_started` when `/billing/checkout/` opens Stripe Checkout, `subscription_started` when the success page first sees the webhook-recorded subscription (both only with analytics consent); shown as "billing not live" until data exists |
...
### Accounts, email verification and Google

Accounts use django-allauth 65 on Django's own `User` (no custom user model). `apps/accounts/adapters.py` is where it fits Corect; the short URL names `login`, `signup` and `logout` point at allauth's views.

- **Sign-in by email, a name that need not be unique.** Sign-in asks for the email address (or Google), never a username (`ACCOUNT_LOGIN_METHODS = {"email"}`). The site shows `User.first_name` ("Numele tău": required, any name, two accounts may share it; editable on the profile). `User.username` stays only as Django's internal unique identifier: allauth generates it at signup and it is never asked for or shown (staff still see it in the admin and analytics). Migration `accounts.0006` copied each existing username into an empty `first_name`, so existing accounts keep the name they chose.

- **Signup.** Name, email, password twice and the unticked "Confirm că am cel puțin 16 ani și accept Termenii și Politica de confidențialitate" checkbox (`apps/accounts/forms.py:SignupForm`, plus a honeypot). The account, its `LegalAcceptance` and its address are saved together; the visitor ID is linked and the consent cookie set, as before. A signup with an address that already has an account creates nothing and shows the same "Verifică-ți emailul" page, while the address's owner gets an "Ai deja un cont" email with a password-reset link (no account enumeration). The database index `accounts_user_email_ci_unique` still decides concurrent signups.
- **One-time verification.** `ACCOUNT_EMAIL_VERIFICATION = "mandatory"`: after signup a link is emailed (`templates/account/email/`); the link opens a page with a "Confirmă" button (`ACCOUNT_CONFIRM_EMAIL_ON_GET = False`, so link scanners cannot confirm) and confirming signs the account in. An address is verified once, in allauth's `EmailAddress` table; later sign-ins need nothing more. Until then the account cannot sign in: signing in again redirects to "Verifică-ți emailul", where a neutral "Nu ai primit emailul?" form (`resend_verification`) sends the link again. allauth's rate limit allows one link per address every few minutes, so neither path can flood an inbox.
- **Changing the address.** On the address page (`/accounts/email/`, linked from the profile as "Schimbă adresa de email") a new address gets its own link and replaces the old one only when confirmed (`ACCOUNT_CHANGE_EMAIL`); the old address keeps working meanwhile. Password and address changes are notified by email (`ACCOUNT_EMAIL_NOTIFICATIONS`). "Ai uitat parola?" on the sign-in page resets it by email.
- **Gate.** `EmailRequiredMiddleware` keeps a signed-in account without a verified primary address on the account pages (profile, address, confirmation, sign-out, legal pages, the Stripe webhook); `/assistant/` gets a JSON 403 `email_required`. Accounts created by staff in the admin or with `createsuperuser` (an address but no allauth row) pass the gate, and allauth still verifies their address at the next password sign-in.
- **Existing accounts (migration `accounts.0005`).** Every account that already had an address gets an `EmailAddress` marked verified and primary, so nobody is locked out, plus a `GrandfatheredEmail` row, because that address was never actually proven. The row disappears when the address is confirmed by a link or by Google. Accounts without an address are untouched, but with sign-in by email they cannot sign in: staff add the address in the admin, then the owner uses "Ai uitat parola?" (which also proves the address). Nothing is invented; running the migration twice adds nothing; it reverses cleanly.
- **Google ("Continuă cu Google").** OAuth with PKCE through allauth's Google provider, a POST button on the sign-in and signup pages, shown only when `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` are set (both or neither: system check `core.E007`). The first Google sign-in always shows the Corect step (`SocialSignupForm`: the name from Google, editable, and the same age/Terms/Privacy checkbox, recorded as a `LegalAcceptance`); an address Google has verified needs no Corect email, an unverified one gets the usual link. A returning Google user signs straight in and accepts nothing again. Linking: a Google-verified address that belongs to an account whose address is verified signs into that account and connects Google (`SOCIALACCOUNT_EMAIL_AUTHENTICATION_AUTO_CONNECT`); never through a grandfathered or unverified address — then no account is created or linked, the owner gets the "Ai deja un cont" email, and after signing in with the password they connect Google from the profile ("Conectează contul Google"). Google-only accounts can set a password on the profile and delete the account by typing their email address.
- **Event screens.** The steps worth marking get a screen in the waiting screen's style (`.wait-screen`: frosted white over the blurred page) with an icon, a title, one line and a button, over the next page: "Bun venit, <name>!" when an account opens (its first address confirmed, or a Google signup with an address Google verified), "Felicitări, acum ești Pro!" the first time the Stripe success page sees the paid subscription active (once per subscription), "Gata!" after the profile, the password (also by reset) or the address changes, and "Rămas bun!" after deletion. `apps/core/events.py:announce()` queues one as a message tagged `event <kind>`, so it survives redirects and the sign-out; `base.html` shows it with `templates/partials/event_screen.html` and every other message as the usual banner; allauth's own banners for these steps are dropped (`AccountAdapter.add_message`). The screen is a `<dialog open>` whose button closes it without JavaScript (`method="dialog"`); `static/js/event-screen.js` makes it modal (focus on the button, Escape or a click beside the text closes it).
- **Email delivery.** Two ways, chosen by the environment. **Resend's HTTPS API** (django-anymail) when `RESEND_API_KEY` is set: the way to send from Railway's Free, Trial and Hobby plans, which block outgoing SMTP; the sender in `DEFAULT_FROM_EMAIL` must be on a domain verified in Resend (Resend → Domains: add `corect.uk`, then the DNS records it shows), or Resend refuses with 403. Otherwise Django's SMTP settings, any provider: `EMAIL_BACKEND` (empty: console with `DJANGO_DEBUG=true`, SMTP otherwise), `EMAIL_HOST`, `EMAIL_PORT`, `EMAIL_HOST_USER`, `EMAIL_HOST_PASSWORD`, `EMAIL_USE_TLS`/`EMAIL_USE_SSL`, `EMAIL_TIMEOUT`, `DEFAULT_FROM_EMAIL`. With DEBUG off, a console backend, a missing host, API key or sender is error `core.E005`. A failed send is logged as `email_send_failed template=… error=… status=…` (no address; `status` is the API's HTTP status, e.g. 403 for an unverified sender domain) and the user sees "Nu am putut trimite emailul acum…", never a 500. There is no marketing email.

### Pro subscriptions (Stripe)

Pro is one monthly Stripe subscription sold through Stripe Checkout (`apps/billing`). `apps/billing/services.py` is the only code that calls Stripe.

- **Checkout.** "Alege Pro" (plan cards) and "Treci la Pro" (profile, "Disponibil în Pro" screens) POST to `/billing/checkout/`: signed-in accounts with a verified address only; anonymous visitors are sent to signup first. The server creates or reuses the account's Stripe customer (metadata `user_id` only; an idempotency key per account), refuses when the account is already Pro or Stripe already has a live subscription for it, expires the account's previous still-open Checkout Session, and opens a `mode=subscription` session for `STRIPE_PRO_PRICE_ID` with success and cancel URLs built from `SITE_URL`. Stripe missing or down: a message, never a 500, and nothing changes.
- **Webhook: the source of truth.** `POST /billing/webhook/stripe/` (CSRF-exempt, no login) verifies the `Stripe-Signature` with `STRIPE_WEBHOOK_SECRET` (400 otherwise), records the event ID in `StripeEvent` (a replay is a no-op) and, under a lock on the account's row, re-reads the subscription from Stripe and stores its status, price, period end and cancel-at-period-end flag in `AccountSubscription`. Because it always stores Stripe's current state, duplicate, late or reordered events cannot bring back a stale one; if Stripe cannot be reached the webhook returns 500 and rolls back, so Stripe's retry is handled in full. Handled events: `checkout.session.completed`, `customer.subscription.created`, `…updated`, `…deleted`, `…paused`, `…resumed`, `invoice.paid`, `invoice.payment_failed`. Logs carry the event type and outcome only. The success page (`/billing/success/`) only reports what the webhook recorded (it refreshes itself while waiting); it never grants Pro.
- **Who is Pro.** `apps/core/plans.py:is_pro()` / `tier_for()`, the one decision point: Pro when the account's subscription status is `active` or `trialing` (`billing.services.ENTITLED_STATUSES`) — a subscription set to cancel at the period end stays Pro until Stripe ends it — or when staff put the account in the "Pro" group (the **manual override** for support and testing; it never creates or changes a subscription; the profile and plan cards say "acordat de echipa Corect.uk"). `past_due`, `unpaid`, `incomplete`, `paused` and `canceled` are Free; there is no local grace period. Quotas, voice and learning guardrails, entitlements and `UsageEvent.plan` all follow `tier_for`, so an upgrade or downgrade applies on the next request and past ledger rows keep the plan they had.
- **Customer Portal.** "Gestionează abonamentul" (profile, plan cards) POSTs to `/billing/portal/`, which opens a Stripe Billing Portal session for the signed-in account's own customer (no identifier ever comes from the browser); card changes, invoices and cancellation happen there.
- **Deleting an account** first cancels, immediately, every subscription of its customer that can still charge; if Stripe cannot be reached (or billing is switched off while a live subscription is recorded), nothing is deleted and the profile says so. The subscription row is deleted with the account; Stripe keeps its invoices.
- **Configuration.** All three of `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRO_PRICE_ID` or none (error `core.E006`); with none, the Pro button says subscriptions cannot be bought right now and nobody can become Pro except through the manual group (warning `core.W002` in production). `STRIPE_PRO_PRICE_ID` must be the recurring monthly price of the Pro product actually on sale, and `PRO_DISPLAY_PRICE`/`PRO_PROMO_PRICE` must state the same amount (they are shown without calling Stripe). No publishable key is needed: Checkout and the Portal are Stripe-hosted pages.
- **Stripe Dashboard.** Create the Pro product with a monthly recurring price (its `price_…` ID is `STRIPE_PRO_PRICE_ID`); enable the Customer Portal (Settings → Billing → Customer portal: allow cancelling at the period end and updating payment methods); add a webhook endpoint `https://<SITE_URL host>/billing/webhook/stripe/` with the events above (its signing secret is `STRIPE_WEBHOOK_SECRET`).
- **Testing with test mode.** Use `sk_test_…` keys and a test price. Locally: `stripe listen --forward-to localhost:8000/billing/webhook/stripe/` prints a `whsec_…` for `STRIPE_WEBHOOK_SECRET`; pay with card `4242 4242 4242 4242`; `stripe trigger customer.subscription.updated` exercises the webhook.
- **Test → live.** Switch all three variables together (live secret key, live price ID, the live endpoint's signing secret) in one deploy; test-mode customers do not exist in live mode, so start from an empty `AccountSubscription` table or delete test rows first. A `sk_test_` key with DEBUG off is warning `core.W003`.

### Railway variables

Everything is configured with environment variables; none are committed. In addition to `DJANGO_SECRET_KEY`, `DJANGO_DEBUG=false`, `DJANGO_ALLOWED_HOSTS`, `DJANGO_CSRF_TRUSTED_ORIGINS` and `SITE_URL` (Django), `DATABASE_NAME`/`USER`/`PASSWORD`/`HOST`/`PORT` (PostgreSQL), `OPENAI_API_KEY`/`OPENAI_MODEL` (OpenAI) and the legal identity (`LEGAL_OPERATOR_NAME`, `LEGAL_SERVICE_ADDRESS`, `CONTACT_EMAIL`), set:

- Email: `RESEND_API_KEY` and `DEFAULT_FROM_EMAIL` (e.g. `Corect.uk <no-reply@corect.uk>`, on the domain verified in Resend). Railway's plans below Pro block SMTP, so the `EMAIL_HOST…` variables only work on Pro or elsewhere.
- Google: `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`. In Google Cloud Console → APIs & Services → Credentials, an OAuth client of type "Web application" with authorised redirect URI `https://<SITE_URL host>/accounts/google/login/callback/` (and `http://127.0.0.1:8000/accounts/google/login/callback/` for a development client); the OAuth consent screen with scopes `openid`, `email`, `profile`, the app name Corect.uk and the privacy and terms URLs.
- Stripe: `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRO_PRICE_ID`.
- Remove `FREE_HAS_PRO_FEATURES` and `PRO_ENTITLEMENTS_ENFORCED` if they are still set (ignored now; warning `core.W001`).

Then deploy, run `python manage.py migrate` (allauth's tables, the grandfathered addresses, billing) and `python manage.py check --deploy`.
...
The V1 settings are intentionally fixed. Live voice input (transcription) and British voice output are included; a realtime voice conversation or speech-to-speech assistant and realtime AI correction are not. Saved activity counts are not an English proficiency score.
