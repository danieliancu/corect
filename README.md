...
- **Email delivery.** Django's email settings from the environment, any SMTP provider: `EMAIL_BACKEND` (empty: console with `DJANGO_DEBUG=true`, SMTP otherwise), `EMAIL_HOST`, `EMAIL_PORT`, `EMAIL_HOST_USER`, `EMAIL_HOST_PASSWORD`, `EMAIL_USE_TLS`/`EMAIL_USE_SSL`, `EMAIL_TIMEOUT`, `DEFAULT_FROM_EMAIL`. With DEBUG off, a console backend, a missing host or sender is error `core.E005`. A failed send is logged as `email_send_failed template=… error=…` (no address) and the user sees "Nu am putut trimite emailul acum…", never a 500. There is no marketing email.
...
- Email: `EMAIL_HOST`, `EMAIL_PORT`, `EMAIL_HOST_USER`, `EMAIL_HOST_PASSWORD`, `EMAIL_USE_TLS` (or `EMAIL_USE_SSL`), `DEFAULT_FROM_EMAIL`.
